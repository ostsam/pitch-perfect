...
export function useRoastSession({
	...
	}, [triggerRoast, pdfTextRef, pageTextsRef, deckSummaryRef, faceDataRef, currentPageRef]);
	...
	useEffect(() => {
		...
		const checkAudioSupport = () => {
			const opusSupported = MediaSource.isTypeSupported('audio/webm; codecs="opus"');
			const mp3Supported = MediaSource.isTypeSupported('audio/mpeg');
			console.log("🎧 Audio Support:", { opus: opusSupported, mp3: mp3Supported });
		};
		checkAudioSupport();
		...
		const startSession = async () => {
			try {
				...
				socket.onmessage = (message) => {
					...
					const isLongBuffer = utteranceBufferRef.current.length - lastAnalyzedLenRef.current > 150;
