...
export function PdfViewer({ file, onPageChange }: PdfViewerProps) {
  ...
  const effectivePageNumber =
    numPages > 0 ? Math.min(pageNumber, numPages) : pageNumber;
  ...
  useEffect(() => {
    onPageChange?.(effectivePageNumber);
  }, [effectivePageNumber, onPageChange]);
    ...
    >
      ...
      >
        <div className="flex items-center gap-2 px-3 py-1.5 bg-black/90 backdrop-blur-xl border border-white/10 rounded-full shadow-lg">
          ...
          <span className="text-[10px] font-medium font-mono text-zinc-500 min-w-8 text-center tabular-nums">
          ...
          <button
            ...
            disabled={effectivePageNumber >= numPages}
            className="p-1.5 hover:bg-white/10 rounded-full text-zinc-400 hover:text-white disabled:opacity-20 transition-colors"
          >
